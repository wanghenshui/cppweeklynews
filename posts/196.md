...
- [Part 4](https://pvs-studio.com/en/blog/posts/cpp/1156/) — Lambda 捕获陷阱、`tuple` 与引用、代理对象（`vector<bool>`）、use-after-move、生命周期延长
  ...
  **生命周期延长**：子对象的引用延长整个父对象的生命周期：
  ...
  User get_user() { return {"Dmitry", {1,2,3,4,5}}; }
...
- [Part 5](https://pvs-studio.com/en/blog/posts/cpp/1160/) — Most Vexing Parse、非常量的常量、move语义、`enable_if_t` vs `void_t`、遗忘return
  ...
  **Most Vexing Parse**：C++ 允许在函数内部前向声明函数，导致构造调用和函数声明的语法冲突：
  ...
  修复：用 `auto w = Worker(Timer())` 或大括号初始化 `Worker w{Timer{}}`。
...
- [Part 6](https://pvs-studio.com/en/blog/posts/cpp/1163/) — 省略号函数、`operator[]`、iostream调试噩梦、逗号运算符、function-try-block、零大小类型
  ...
  **`operator[]` 关联容器**：`map::operator[]` 找不到 key 时自动插入默认值，对不可默认构造的类型编译失败：
  ...
  ```cpp
  std::map<std::string, std::string> options{{"max_value", "1000"}};
