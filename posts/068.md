...
标准库/boost实现的unordered map不是用的开链表，用的closed address，（常规实现是open address又叫做开链，发生冲突，冲突节点整个链表串起来。这种closed address就是发生冲突找个没用的先放着。如果滚动冲突会很惨）

这种如果发生大量冲突, 空间占用/放大是非常恐怖的
...
话说当年为啥hashmap采用closed address而不是open address呢。当年开链法cache不友好性能不行？小编没有深究，
